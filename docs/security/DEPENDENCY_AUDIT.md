...
| CycloneDX generation       | Spec 1.5, 1,147 unique package-version components               |
...
| A Windows-authored npm lockfile omitted Astro's transitive Apple Silicon compiler and Satteri packages, so separate EAS source gates could not complete site typechecking and production build.                                           | Closed                      | Declare exact optional `@astrojs/compiler-binding-darwin-arm64@0.4.0` and `@bruits/satteri-darwin-arm64@0.10.5`. Windows skips the binaries; macOS npm ci installs the locked matching packages.                     |

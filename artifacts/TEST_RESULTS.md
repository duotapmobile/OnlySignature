...
| `npm run test --workspace @only-signature/mobile` | PASS: 17/17 tests |
| `npx expo install --check` | PASS: dependencies current for Expo SDK 57 |
| `npx expo-doctor` | PASS: 21/21 checks |
| `npx expo export --platform ios --output-dir dist-ios` | PASS: 25 local bundle/assets files; not a signed build |
...
| `node scripts/check-content-drift.mjs` | PASS: 108 source/content files |
...
| screenshot composition | PASS: 8 iPhone + 8 iPad web-rendered fixture masters, exact dimensions, no alpha; final iOS capture remains gated |
